...
void MagneticEffect::initialize(cWorld* world, cCamera* camera)
{
    ...
    cShapeLine* myLine = new cShapeLine(cVector3d(0, 0.0, 1), cVector3d(0, 0.0, -1));
...
void MagneticEffect::updateHaptics(cGenericHapticDevice* hapticDevice, double timeStep, double totalTime)
{
    ...
    if (newPosition.y() > -0.05 && newPosition.y() < -0.01) {
        force.y(-1.5 / (-0.05 + 0.01) * (newPosition.y() - (-0.01)) + 1.5);
    }
    else if (newPosition.y() < 0.05 && newPosition.y()  > 0.01) {
        force.y(1.5 / (0.05 - 0.01) * (newPosition.y() - (0.01)) + -1.5);
    }
    else if (cAbs(newPosition.y()) < 0.01) {

        force.y(-1.5 / (0.01) * newPosition.y());
    }
